...
<body style="background-color: rgba(224, 224, 224, 0.2);">
	...
	%>
	<p class="logo">
		<a href="../main.jsp" style="color: rgb(164, 214, 82)">Assemble</a>
	</p>
	<div class="container" style="margin-top: 100px;">
		<div class="row">
			...
			<thead>
				<tr>
					<th colspan="4" style="text-align: center; font-size: 20px;"><%= name + " 게시글"%></th>
				</tr>
			</thead>
			<tbody>
				<tr>
				...
				<tr>
					<td>작성자 : </td>
					<td colspan="3" style="text-align: left;"><%= bbs.getUserID() %></td>
				</tr>
				<tr>
					<td>작성일자 : </td>
				...
				<tr>
					<td>오픈채팅URL : </td>
					<td colspan="3" style="text-align: left;"><%= bbs.getBbsOpenChat() %></td>
				</tr>
				<%
					}
				%>
			...
			<a href="javascript:window.history.back();" class="btn btn-primary" style="background-color: rgb(164, 214, 82); border-color: black; width: 90px; color: white">뒤로가기</a>
			<%
				if(userID != null && userID.equals(bbs.getUserID())) {
			%>
					<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="deleteAction.jsp?partyNum=<%= partyNum%>" 
						class="btn btn-primary pull-right" style="background-color: rgb(164, 214, 82); border-color: black; width: 60px; color: white;">삭제</a>
					<a href="update.jsp?partyNum=<%= partyNum%>" class="btn btn-primary pull-right" 
						style="background-color: rgb(164, 214, 82); border-color: black; width: 60px; color: white; margin-right: 8px;">수정</a>
					<a href="member.jsp?partyNum=<%= partyNum%>" class="btn btn-primary pull-right" 
						style="background-color: rgb(164, 214, 82); border-color: black; width: 75px; color: white; margin-right: 8px;">파티원</a>
			<%
				} else {
			...
			%>
				<a onclick="return confirm('가입신청 하시겠습니까?')" href="partyJoinCheck.jsp?partyNum=<%= partyNum%>" 
					class="btn btn-primary pull-right" style="background-color: rgb(164, 214, 82); border-color: black; width: 100px; color: white;">파티 가입신청</a>
			<%
						}
					} if(partyCheck == 1){
			%>
						<a onclick="return confirm('탈퇴하시겠습니까?')" href="partyOutAction.jsp?partyNum=<%= partyNum%>" 
							class="btn btn-primary pull-right" style="background-color: rgb(164, 214, 82); border-color: black; width: 90px; color: white;">파티탈퇴</a>
			<%	
					}
				}
			%>
