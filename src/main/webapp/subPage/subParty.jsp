...
<body style="background-color: rgba(224, 224, 224, 0.2);">
	...
	<div id="total">
		<div id="nav"></div>
		<div id="total-right">
			<div id="section-blank"><h1 style="margin-left: 130px; margin-top: 50px;">가입 파티 목록</h1></div>
			<div id="footer">
				<%
					...
					for (int i = 0 + (pageNumber - 1) * 12; i < number; i++) {
						if(list.get(i).getCategoryName().equals("ETC")){
							name = "기타";
						} else {
							name = list.get(i).getCategoryName();
						}
				%>
				<ul class="party" type="none">
					<li style="height: 100%;">
						<div class="top" style="cursor: pointer; color: red;" onclick="location.href='view.jsp?partyNum=<%= list.get(i).getPartyNum()%>';"><%= name%></div>
